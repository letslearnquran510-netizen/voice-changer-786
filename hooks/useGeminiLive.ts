...
export const useGeminiLive = ({ apiKey, vadThreshold = 0.01 }: UseGeminiLiveProps) => {
  ...
  const disconnect = useCallback(async () => {
    ...
    if (fileSourceRef.current) {
        try { fileSourceRef.current.stop(); } catch (e) {}
  ...
  const connect = useCallback(async (persona: Persona, file?: File, language: LanguageCode = 'en') => {
    ...
    try {
      ...
      } catch (e) { console.warn("Recorder failed", e); }
      ...
      if (file) {
        ...
        const stream = await navigator.mediaDevices.getUserMedia({ 
            audio: { sampleRate: 16000, echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 1 } 
        });
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
                ...
                currentInputIdRef.current = ''; currentInputTextRef.current = '';
                currentOutputIdRef.current = ''; currentOutputTextRef.current = '';
                nextStartTimeRef.current = 0; 
            }
          },
  ...
  return { connect, disconnect, status, error, activePersona, mediaStream: audioStream, inputVolume, isVadActive, downloadUrl, transcripts };
};
